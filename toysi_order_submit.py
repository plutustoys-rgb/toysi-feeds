...
def submit_order(order: dict, test_mode: bool = False) -> dict:
    ...
    return {
        ...
        "is_duplicate": code == 2,
        # code 3 — Toysi сам рекомендує повторити. code 5 ("Невірний auth_user або
        # auth_key") — це помилка АВТОРИЗАЦІЇ/КОНФІГУ (протух/невірний ключ у .env),
        # а НЕ браковані дані замовлення. Раніше code 5 падав у термінальну гілку
        # order_router → замовленню виставлявся status='toysi_error' → воно НАЗАВЖДИ
        # випадало з черги (get_orders_ready_to_forward виключає 'toysi_error'), навіть
        # після виправлення ключа (інцидент 904194938, 2026-08-25: живе замовлення
        # 704₴ застрягло, довелось вручну скидати статус). Трактуємо як should_retry:
        # замовлення лишається в черзі й саме доїде в Toysi, щойно ключ полагодять.
        # Дублювання не буде — при невірному ключі Toysi нічого не створює. Про
        # застряглий на code 5 форвард окремо кричить throttled-алерт з order_router.
        # Коди 4, 6-21 (браковані ДАНІ замовлення) лишаються термінальними — там
        # повтор безглуздий, потрібне ручне втручання.
        "should_retry": code in (3, 5),
        "raw": data,
    }
