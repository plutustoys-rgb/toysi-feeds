...
WATCHERS = [
    ...
    },
    {
        ...
        # ⚠️ БЕЗ слеш-команд: у headless `claude -p` = «Unknown command» → no-op. Роль текстом, self-contained.
        # ⚠️ Браузера тут НЕМАЄ (фонове пробудження — файлові інструменти, той самий структурний ліміт,
        # що й у Виконавця/Продажника: mcp__Claude_Browser__* повертають no_tool навіть з --agent).
        # Замовлення (6) просило браузер «для вибіркової перевірки» — headless фізично не може його
        # дати; це ЧАСТКОВЕ виконання, звірка з живими сторінками вимагає окремої повної сесії
        # (панель), не цього вотчера — сказано прямо в periodic_prompt нижче, не замовчено.
        "wake_prompt": (
            ...
            "власник відкриє» і зупинись, не пиши висновок без перевірки. НЕ пиши в канал твердження про "
            "ТЕ, ЯК влаштована система, якого сам не перевірив — гіпотеза ≠ факт, агент-заяви ≠ істина. "
            "Розвилки/рішення власника → OWNER_INBOX.md."
        ),
        "periodic_prompt": (
            ...
            "4) реакція живої людини на заявку → негайно `## [Консультант → Код]` в OWNER_INBOX.md; "
