...
- якщо floor > кандидат (навіть на 1 копійку — ЖОДНОЇ "грації") -> "delist",
  приховування товару з вітрини (presence=not_available, НЕ status=deleted
  — див. докстрінг delist() нижче: живо перевірено 2026-07-18, ціна
  запиту власниці "як зробити це безболісно для рейтингу"). Раніше тут
  була "грація" 1.5x і повне виключення buyBox-джерела з delist — обидва
  прибрано, вони і були причиною системного розриву 10-31%.
...
def delist(external_id: str) -> None:
    """ВИПРАВЛЕНО (2026-07-18, пряме запитання власниці "як зробити це
    безболісно для рейтингу?"): раніше — status="deleted" (той самий
    виклик, що й prom_catalog_sync.py) — ПОВНЕ видалення оголошення,
    втрата накопиченого рейтингу/відгуків, не відкочується автоматично.

    Тепер — presence="not_available", НЕ status. Живо перевірено
    2026-07-18 на реальному SKU 265230 (встановлено, підтверджено через
    GET, одразу відкочено назад на "available"): товар зникає з пошуку/
    неможливий до покупки, АЛЕ status лишається "on_display" — саме
    оголошення НЕ видаляється, тож рейтинг/відгуки/історія зберігаються.
    Коли товар знову стане конкурентним (ціна конкурента зміниться) —
    досить повернути presence="available" на ТОМУ Ж оголошенні, без
    повторного створення й "холодного старту" рейтингу з нуля.

    prom_catalog_sync.py (окремий скрипт, деактивація за відсутністю
    стоку в Toysi, не за ціною) НАВМИСНО не займано цим фіксом — інше
    призначення (товар більше НІКОЛИ не постачається), тут не питання."""
    response = requests.post(
        f"{PROM_API_URL}/products/edit_by_external_id",
        headers={"Authorization": f"Bearer {PROM_API_KEY}"},
        json=[{"id": external_id, "presence": "not_available"}],
        timeout=REQUEST_TIMEOUT,
    )
    response.raise_for_status()
