...
# ліміт створення товарів обидва підняті до 6000 — старт поетапного
# розширення (970 -> 2000 -> 4000 -> 6000, не одним стрибком). Репрайсер
# (prom_competitor_pricer.py) тепер обробляє SELECT_COUNT ротаційними
# партіями по ROTATION_BATCH_SIZE щочотири години (не все одразу раз на
# добу) — див. коментар там же щодо чому.
TARGET_COUNT    = 2000
