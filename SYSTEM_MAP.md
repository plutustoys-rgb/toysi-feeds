...
> Знято бандлом `systemctl list-timers/list-unit-files/cat` (вивід власника 2026-08-20). Розклади
> (OnCalendar) — окремо в `list-timers`; тут фіксуємо ЩО і чим. Перший прогін `system_map_driftcheck.py`
> НА VPS підтвердить повноту (якщо якийсь юніт видно не було — drift-check його висвітить).
