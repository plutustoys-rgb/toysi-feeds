...
- Основний і повністю автоматизований канал — Нова Пошта. Замовлення,
  оформлені до 12:00 у робочі дні, відправляються того ж дня.
...
DELIVERY_ANSWER = (
    "Відправляємо Новою Поштою — замовлення, оформлені до 12:00 у робочі "
...
# Матч ВХОДЖЕННЯМ (2026-09-04, наказ власника «щоб бот давав відповідь на такі повідомлення»):
# раніше шаблон вимагав ТОЧНОГО збігу всього нормалізованого повідомлення (`normalized in SET`),
# тож живі питання з привітанням («Доброго дня. Коли можлива відправка?») ніколи не збігались і
# йшли в ескалацію → у чаті тиша. Тепер додано матч по інтент-СТЕМАХ (підрядок), із гейтом-
# запобіжником проти скарг: якщо в тексті є маркер скарги/дефекту/повернення/затримки — шаблон
# НЕ спрацьовує (бадьора відповідь на скаргу = репутаційна шкода), лишаємо на ескалацію власнику.
# Стеми curated: короткі двозначні («чи є», голе «ціна») свідомо НЕ включені, щоб не спрацьовувати
# на «чи є відео?» чи «ціна завелика, поверніть».
_DELIVERY_STEMS = (
    "доставк", "відправ", "поштою", "нова пошта", "новою пошт", "перевізник",
    "з якого міста", "спосіб доставки", "способи доставки",
)
_STOCK_STEMS = ("наявн", "залишок", "є в наявн", "в наявн")
_PRICE_STEMS = ("яка ціна", "скільки кошту", "вартіст", "почім", "по чому", "ціна товар", "ціна на")
# Скарга/дефект/повернення/затримка/недоставка — укр.+рос. (Prom часто рос.-мовний). Якщо є хоч один —
# НЕ шаблонимо, віддаємо на ескалацію. Ширший набір, ніж хочеться помилитися в інший бік.
_TEMPLATE_COMPLAINT_MARKERS = (
    "поверн", "верну", "гаранті", "гаранти", "не працю", "не робот", "брак", "бракован",
    "злама", "сломал", "розбит", "разбит", "пошкодж", "поврежд", "повредж", "несправн", "неисправ",
    "дефект", "мят", "м'ят", "брудн", "грязн", "запізн", "опозда", "затрим", "задерж",
    "не той", "не таке", "не те", "не так", "не тот", "не така", "недостач", "не вистача", "не хвата",
    "не вклали", "не доклали", "не поклали", "не полож", "скарг", "жалоб", "проблем", "плох",
    "не сподоб", "не понрав", "розчаров", "разочаров", "обурен", "жахлив", "ужасн",
    "не відправ", "не надісл", "не прийш", "не дійш", "де моє замовл", "де моя посилк", "загуб", "втрат",
)


def _has_complaint(normalized: str) -> bool:
    """True, якщо в тексті є маркер скарги/дефекту → шаблон НЕ застосовуємо (ескалація)."""
    return any(m in normalized for m in _TEMPLATE_COMPLAINT_MARKERS)


def _match_stems(normalized: str, exact_set: frozenset | set, stems: tuple) -> bool:
    """Збіг як за старим ТОЧНИМ списком фраз, так і за новими інтент-стемами (підрядок)."""
    return normalized in exact_set or any(s in normalized for s in stems)

# Подяка/схвалення — авто-відповідь БЕЗ LLM. Раніше без ANTHROPIC_API_KEY (якого проєкт не вживає)
# такі повідомлення ескалювались, бо це не питання-шаблон (кейс Інни Шепотько 2026-09-04, власник
# схвалив авто-подяку). ЗАХИСТ: спрацьовує ЛИШЕ коли є маркер вдячності І НЕМА питання/скарги — щоб
# не «дякувати» на брак і не глушити реальне питання (ті йдуть далі у шаблон/ескалацію).
_ACK_MARKERS = (
    "дякую", "дякуємо", "спасибі", "спасибо", "отримали", "отримав", "получили", "получила",
    "все добре", "все гаразд", "все гарно", "гарно упакова", "все ок", "все окей", "супер",
    "чудово", "класно", "задоволен", "вдячн", "все чудово", "все супер", "все прийшло",
    "в захваті", "у захваті", "в восторге", "прекрасно", "рекомендую",
)
_ACK_STOP_MARKERS = (
    # контраст-слова: будь-яка «подяка», що містить «але/но/однак/тільки/только», — це майже завжди
    # застереження/скарга → ескалюємо, НЕ дякуємо (безпечний широкий гейт; у сумніві — не авто-текст).
    " але ", " але,", " но ", " но,", "однак", "тільки ", "только ", "хоча ",
    # питання
    "?", "чи ", "як ", "коли", "скільки", " де ", "можна", "хочу", "потрібно", "підкажіт",
    "цікавить", "наявн", "колір", "кольор", "розмір", "ціна", "вартіст", "оплат", "обмін",
    "чому", "почему", "цікаво", "хотіла б", "хотів би", "де моє", "де замовл",
    # скарги/дефект — укр. І рос. форми (Prom дуже часто рос.-мовний)
    "поверн", "верну", "гаранті", "гаранти", "не працю", "не робот", "брак", "бракован",
    "злама", "сломал", "розбит", "разбит", "пошкодж", "поврежд", "повредж", "несправн", "неисправ",
    "дефект", "мят", "м'ят", "брудн", "грязн", "запізн", "опозда", "затрим", "задерж",
    "не той", "не таке", "не те", "не так", "не то ", "не тот", "не така",
    "недостач", "не вистача", "не хвата", "не вклали", "не доклали", "не поклали", "не полож",
    "скарг", "жалоб", "проблем", "якіс", "якост", "качеств", "плох", "погано", "погана", "поган",
    "не сподоб", "не понрав", "розчаров", "разочаров", "обурен", "жахлив", "ужасн",
)
ACK_ANSWER = (
    "Дякуємо за відгук! 🦊 Раді, що все добре. Гарних ігор малечі — і будемо вдячні за "
    "відгук про товар, якщо буде хвилинка. З теплом, PlutusToys."
)


def _try_ack_response(customer_message: str) -> str | None:
    """Авто-подяка на чисте схвалення. None, якщо нема маркера вдячності АБО є питання/скарга."""
    low = (customer_message or "").lower()
    if not any(m in low for m in _ACK_MARKERS):
        return None
    if any(s in low for s in _ACK_STOP_MARKERS):
        return None
    return ACK_ANSWER
...
# ВИПРАВЛЕНО (незалежне рев'ю PR #90, code_report pt14): _COLOR_INTENT_
# MARKERS сам по собі занадто широкий — "хочу"/"можна"/"хочемо" природно
# трапляються і в скарзі/поверненні ("хочу повернути синій самокат, він
# зламаний", "можна поміняти на інший колір, цей бракований"), не лише в
# запиті на замовлення. Без цього списку такі повідомлення хибно
# отримали б життєрадісну "так, є в наявності" — пряму, немодеровану
# відповідь клієнту (STORE_POLICY вище явно вимагає ескалації для
# повернень/скарг). Перевірка нижче — ВИНЯТОК: якщо є хоч один такий
# маркер, шаблон НЕ спрацьовує НЕЗАЛЕЖНО від позитивних маркерів вище.
_COLOR_COMPLAINT_MARKERS = {
    "повернути", "повернення", "обміняти", "обмін", "зламаний", "зламана",
    "зламане", "поламаний", "поламана", "поламане", "бракований",
    "бракована", "браковане", "брак", "скарга", "неробочий", "неробоча",
    "неробоче",
}
...
def _detect_requested_color(customer_message: str) -> str | None:
    ...
    РІВНО ОДИН явний колір І є маркер наміру запитати/замовити, І НЕМАЄ
    жодного маркера скарги/повернення (див. коментарі вище). Кілька
    різних кольорів, жодного маркера наміру, чи будь-який маркер скарги —
    None, свідомо не вгадуємо."""
    tokens = _normalize(customer_message).split()
    if any(t in _COLOR_COMPLAINT_MARKERS for t in tokens):
        return None
...
# ВИПРАВЛЕНО (незалежний аудит PR #155, code_report pt6): перший варіант
# мав ДРУГИЙ, значно ширший патерн `\bце\s+(.+?)\?` — ловив БУДЬ-ЯКЕ "це
# ...?" будь-де в реченні, не лише про товар. Живе відтворення аудиту:
# "це можна оплатити карткою?", "скажіть, це вже відправили?", "це
# нормально, що замовлення ще не прийшло?" — жодне не про ідентичність
# товару, усі хибно ловились і псували відповідь. Прибрано повністю,
# лишився лише вужчий "чи це ...?".
#
# Але й "чи це" саме по собі НЕ гарантія — аудит показав живий приклад
# "чи це доставка не затримується?", де слово "чи" присутнє, а тема —
# доставка, не товар. Тому ДОДАНО другий, незалежний гейт нижче
# (_IDENTITY_OFF_TOPIC_MARKERS) — явний список тем, які напевно НЕ про
# товар (доставка/оплата/терміни), той самий принцип явних маркерів, що
# й _IDENTITY_COMPLAINT_MARKERS вище, а не спроба вгадати семантику.
_IDENTITY_QUESTION_PATTERNS = [
    re.compile(r"чи\s+це\s+(.+?)\?", re.IGNORECASE | re.UNICODE | re.DOTALL),
]

_IDENTITY_OFF_TOPIC_MARKERS = {
    "доставка", "доставку", "доставки", "доставили", "доставлять", "доставлено",
    "відправили", "відправлять", "відправлено", "відправите", "відправите",
    "оплата", "оплату", "оплатити", "оплатили", "картка", "карткою", "картою",
    "готівкою", "накладеним", "накладений", "платежем", "затримується",
    "затрималось", "затримали", "затримка", "прийшло", "прийде", "отримаю",
    "отримали", "поштою", "пошта", "накладеною",
}
...
def _detect_identity_claim(customer_message: str) -> str | None:
    ...
    питання "чи це X?", або None — якщо повідомлення не така форма
    питання, чи містить маркер скарги/явно не про товар (тоді свідомо не
    чіпаємо, хай іде до ескалації/LLM, а не даємо життєрадісну
    автовідповідь на скаргу "прийшло не те" чи питання про доставку)."""
    if "?" not in customer_message:
        return None
    tokens = _normalize(customer_message).split()
    if any(t in _IDENTITY_COMPLAINT_MARKERS for t in tokens):
        return None
    if any(t in _IDENTITY_OFF_TOPIC_MARKERS for t in tokens):
        return None
...
def try_template_response(customer_message: str, product_context: dict | None) -> str | None:
    ...
    ack = _try_ack_response(customer_message)
    if ack is not None:
        return ack

    # Скарга/дефект/повернення/затримка → НЕ шаблонимо жодним шаблоном, віддаємо на ескалацію
    # (бадьора шаблонна відповідь на скаргу = шкода). Стоїть ПЕРЕД усіма шаблонами нижче.
    if _has_complaint(normalized):
        return None

    if _match_stems(normalized, DELIVERY_PHRASES, _DELIVERY_STEMS):
    ...
    if _match_stems(normalized, STOCK_PHRASES, _STOCK_STEMS):
    ...
    if _match_stems(normalized, PRICE_PHRASES, _PRICE_STEMS):
