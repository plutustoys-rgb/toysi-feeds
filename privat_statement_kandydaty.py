...
(виписка покриває дохід/комісії/інші списання одразу), а в БУДЬ-ЯКІЙ грошовій графі книги
(2,3,4,6,7,8,9,10,11) на відповідну дату — бо яка саме графа підходить конкретній операції,
...
# Грошові графи книги (0-indexed, ряд iter_rows(min_row=7)): 1=дохід,2=повернення,3=до
# перерахування,5=реалізація,6=разові акти,7=ФОП/банк.коміс,8=інше списання,9=амортизація,
# 10=резерв. Індекс 0=дата (графа1, ключ звірки), 4=джерело(графа5, текст), 11=примітки(графа12,
# текст) — НЕ грошові, у цей індекс не входять.
_BOOK_MONEY_COLS = (1, 2, 3, 5, 6, 7, 8, 9, 10)
...
def main() -> None:
    ...
    lines = [
        ...
        "",
        "«У книзі» — сума операції знайдена в БУДЬ-ЯКІЙ грошовій графі книги (2,3,4,6,7,8,9,10,11)",
        "на ту саму дату ±1 день. Кирилична деталізація (призначення/контрагент) у файлі кандидатів",
        "(json поруч), сюди не виносимо — короткий огляд.",
        "",
