...
WATCHERS = [
    {
        ...
        "schedule": None,
        # ⚠️ cwd=COWORK (2026-08-21, ВИПРАВЛЕНО): Код стартує в папці каналів (як SEO/SMM). Раніше був у
        # репо (BASE_DIR) заради CLAUDE.md, а канали через --add-dir → headless упирався в дозвіл-промпт
        # і НЕ МІГ ПРОЧИТАТИ канали взагалі (діагностовано живо: сам headless-Код сказав «can't reach the
        # channel files»). Тобто Код не бачив запитів → «безрукий». Тепер канали читає з cwd, а РЕПО чіпає
        # через Bash (Bash не гейтиться по шляху — перевірено живо: git -C <репо> працює з cwd=COWORK).
        "cwd": str(COWORK_DIR),
        # B (2026-08-21): +Bash — щоб збирати код і відкривати PR. Мерж headless заблоковано хуком
        # merge-guard через env PLUTUS_AGENT_HEADLESS. Файли РЕПО редагувати ЧЕРЕЗ Bash (Edit-tool поза cwd
        # блокується), канали — Edit-tool (вони в cwd).
        "allowed_tools": ["Read", "Edit", "Write", "Glob", "Grep", "Bash"],
        "wake_prompt": (
            "Ти — сесія «Код» проєкту PlutusToys. Монітор виявив НОВИЙ запис, адресований Коду. "
            ...
            "сесія/власник (headless-мерж заблоковано хуком). Якщо запис — підтвердження/закритий, нічого "
            "не роби. Незворотне/публічне без власника — ні; cost/margin у спільні файли — ні; рішення "
            "власника → OWNER_INBOX.md."
        ),
    },
    {
        "name": "SEO",
