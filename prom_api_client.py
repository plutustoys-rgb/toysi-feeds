...
def edit_by_external_id(payload: dict) -> None:
    ...
    # ДОДАНО (2026-07-21, дослідження "чому Prom стабільно відхиляє
    # видалення саме SKU 201887/185297" — findings_log.md
    # "prom-delete-still-rejected-for-specific-sku"): живо перевірено —
    # Prom для цих SKU повертає ГЕНУЇННО порожню відповідь,
    # `{"processed_ids": [], "errors": {}}`, без жодної причини десь-інде
    # в тілі — це НЕ баг парсингу нижче (немає прихованого повідомлення,
    # яке ми губимо), а справжній мовчазний no-op на боці Prom. Сира
    # відповідь друкується завжди, коли швидкий шлях (processed_ids) не
    # спрацював — щоб будь-який МАЙБУТНІЙ подібний випадок (можливо, з
    # реальною причиною в іншому полі) не довелось діагностувати вручну
    # з нуля.
    print(f"[PromAPI] {ext_id}: processed_ids не містить цей id, сира відповідь: "
          f"{json.dumps(data, ensure_ascii=False)[:1000]}", file=sys.stderr)
