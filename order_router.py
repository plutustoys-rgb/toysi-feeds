...
def parse_np_branch(np_branch: str) -> tuple:
    ...
    (напр. "Київ, відділення №15" з мок-даних, або delivery_address з
    реального Prom).
    ...
    Київ (Київська обл.)") — з відсіканням резолвиться коректно, і
    отримані city_ref/warehouse_ref точно збігаються з city_id/
    recipient_warehouse_id, які сам Prom вже повернув у замовленні.
    """
    if not np_branch:
        return "", ""
