...
⚠️ ВІДКРИТЕ ПИТАННЯ (з 2026-07-08, не закрито): "ціна_конкурента" вище — це
ОДНЕ спільне число на товар (min_competitor_raw в --record), знайдене
ВИКЛЮЧНО на Rozetka.ua (дивись докстрінг вище) і застосоване до ОБОХ
decide_price_for_platform() викликів однаково (prom і rozetka). Окремого
механізму пошуку ціни конкурента САМЕ на Prom.ua не існує ніде в проєкті —
жодного скрипта, жодного API-виклику. Для SKU без ручного запису в
pricing_results.csv (переважна більшість — на 2026-07-09 записано лише 19
товарів) ціна для Prom рахується як max(нижня_межа, cost*NO_COMPETITOR_MULT)
— це чисто формульна (cost-plus) ціна, БЕЗ жодного живого сигналу про
реальні ціни конкурентів на Prom.ua. Rozetka зараз неактивна (магазин не
зареєстрований), тож фактично увесь конвеєр --record сьогодні шукає
конкурента на майданчику, де ми ще не продаємо, і переносить цей результат
на Prom, де продаємо. Чи потрібен окремий Prom-специфічний механізм пошуку
конкурента (ручний за аналогією з Rozetka, чи інший) — рішення власника,
не приймалось.
