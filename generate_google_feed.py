...
# ДОДАНО (2026-07-20, аудит PR #109 — code_report_2026-07-20_pt11.md):
# fetch_prom_products() (повний список товарів кабінету, /groups/list +
# /products/list по кожній групі, БЕЗ жодного власного кешування —
# перевірено напряму в prom_catalog_sync.py) раніше викликався
# НЕЗАЛЕЖНО в КОЖНОМУ з трьох феєд-скриптів (Google/Meta/Bing) в
# ОДНОМУ прогоні update-feeds.yml — потрійне навантаження на Prom API,
# той самий клас проблеми, якого PR #109 явно уникнув для self-match
# пошуку (own_product_links_cache.json), але не поширив на цей виклик.
# TTL навмисно короткий (1 година, не 7 днів, як у кешах вище) — це
# суто внутрішньопрогонна оптимізація (кеш завжди пишеться заново на
# початку кожного окремого прогону CI), не довгостроковий кеш між
# прогонами: дані про ціну/залишок мають лишатись максимально свіжими.
PROM_PRODUCTS_CACHE_FILE = Path(__file__).parent / "prom_products_raw_cache.json"
PROM_PRODUCTS_CACHE_TTL_HOURS = 1


def save_prom_products_cache(prom_products: dict) -> None:
    try:
        PROM_PRODUCTS_CACHE_FILE.write_text(
            json.dumps(prom_products, ensure_ascii=False), encoding="utf-8"
        )
    except OSError as e:
        print(f"[Google] Не вдалось зберегти кеш товарів Prom ({e}) — не критично, "
