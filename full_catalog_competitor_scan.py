"""
full_catalog_competitor_scan.py — безкінечний, поступовий конкурентний
скан ПОВНОГО каталогу Toysi, готуючи дані для побудови нового топ-1000.

ЦИКЛ НЕ ЗУПИНЯЄТЬСЯ (ДОДАНО 2026-07-21, пряме прохання власниці —
живий приклад SKU 296283, ціна 58% дорожча за конкурента, виявлена
власноруч: "я хочу щоб скан не зупинявся... товари можуть з'являтися
нові, а ми про них не дізнаємось"): до цього фіксу скрипт після
повного покриття обсягу просто зупинявся НАЗАВЖДИ — жодне подальше
оновлення конкурентних даних для вже сканованих SKU не відбувалось,
поки хтось вручну не видалив стан. Тепер два режими одного циклу —
див. коментар біля вибору batch_ids у main(): "початкове покриття"
(ще не всі SKU бачили хоч раз) переходить у "цикл освіження"
(пересканування найстаріших записів, щойно повне покриття досягнуто),
який ніколи не завершується сам по собі.
...
competitor_alive, competitor_image, margin_pct, price_category,
scanned_at}}. Поки є SKU, ще НІ РАЗУ не сканований (новий товар у
Toysi чи перший прохід) — кожен запуск бере наступні BATCH_SIZE таких
(стабільний порядок за ідентифікатором — тут немає "пріоритету за
маржею", бо мета саме уникнути попередньої маржинальної фільтрації).
Щойно всі SKU обсягу побачені хоч раз — цикл переходить у безкінечне
освіження: кожен запуск бере BATCH_SIZE записів із НАЙСТАРІШИМ
scanned_at і пересканує їх, тож жоден SKU не лишається без оновлення
назавжди. Ідемпотентно, безпечно перезапускати в обох режимах.
...
# навантаження репрайсера, ~2.1 год/день, у межах 180-хв safety timeout
# workflow'у.
#
# ПІДВИЩЕНО 3000->4000 (2026-07-22, пряме прохання власниці "чіткіше
# знаходити конкурентів" — швидше покриття всього каталогу): бенчмарк
# 2.521с/SKU (docstring файлу вище) дає ~168 хв на 4000 SKU — досі з
# запасом ПІД 180-хв safety timeout (не впритул), ~33% швидше покриття
# (повне 29173-SKU покриття скорочується приблизно з ~6 до ~4.3 днів
# першого проходу). Ризик rate-limit за 5.5 тижнів роботи на 3000/день
# жодного разу не підтвердився живо — консервативний, але не надмірний
# крок вгору.
BATCH_SIZE = 4000
...
def main() -> None:
    ...
    # ДОДАНО (2026-07-21, пряме прохання власниці — живий приклад SKU
    # 296283 "Фігурки Гаррі Поттер", ціна 1314.44 грн проти конкурента
    # 829 грн, 58% дорожче: "я хочу щоб скан не зупинявся, навіть після
    # завершення прогона по всьому каталогу... товари можуть з'являтися
    # нові, а ми про них не дізнаємось"): раніше, щойно обсяг був
    # повністю покритий один раз, скрипт просто зупинявся НАЗАВЖДИ
    # (`return` нижче) — жодне подальше оновлення конкурентних цін для
    # вже сканованих SKU не відбувалось, доки хтось вручну не видалив
    # би стан. Тепер два режими одного й того самого циклу:
    #
    # 1. "початкове покриття" (remaining_ids непорожній) — та сама
    #    логіка, що й раніше: стабільний порядок за ID, без пріоритету
    #    за маржею. Сюди автоматично потрапляють і НОВІ товари (scope
    #    рахується напряму з ЖИВОГО каталогу Toysi щоразу, тож SKU,
    #    доданий постачальником з часу минулого прогону, опиняється в
    #    remaining_ids сам по собі, без жодної спеціальної обробки).
    # 2. "цикл освіження" (remaining_ids порожній — повне покриття вже
    #    досягнуто) — замість зупинки скрипт освіжає BATCH_SIZE записів
    #    із НАЙСТАРІШИМ `scanned_at`, обмежених поточним scope (товар,
    #    що випав зі stock/став уцінкою, природно випадає й тут, без
    #    додаткової логіки очищення). Записи БЕЗ scanned_at (дані до
    #    цього фіксу) рахуються найстарішими — освіжаться першими.
    #    Цикл ніколи не завершується сам по собі: щойно останній запис
    #    освіжено, він одразу стає НАЙНОВІШИМ scanned_at, і наступний
    #    прогін природно перейде до тепер-найстарішого — вічний round-robin.
    if remaining_ids:
        # Стабільний порядок (за ID) — навмисно БЕЗ пріоритету за маржею,
        # на відміну від попередньої версії плану: мета саме уникнути
        # будь-якої попередньої економічної фільтрації/сортування до
        # того, як зберемо дані по ВСЬОМУ обсягу.
        ordered_remaining = sorted(remaining_ids)
        batch_ids = ordered_remaining[:args.batch_size]
        mode = "початкове покриття"
    else:
        refreshable = sorted(
            (pid for pid in state if pid in scope),
            key=lambda pid: state[pid].get("scanned_at") or "",
        )
        batch_ids = refreshable[:args.batch_size]
        mode = "цикл освіження (повне покриття вже досягнуто)"

    print(f"[FullScan] Режим: {mode}.")
    print(f"[FullScan] Пакет цього запуску: {len(batch_ids)} SKU "
          f"(~{len(batch_ids) * 2.521 / 60:.1f} хв за виміряним темпом 2.521с/SKU).")

    if not batch_ids:
        print("[FullScan] Нема що сканувати цього разу (порожній scope?) — зупиняюсь.")
        return
    ...
    save_state(state)
    # ВИПРАВЛЕНО (2026-07-21, разом із введенням циклу освіження): раніше
    # `len(scanned_ids) + len(batch_ids)` неявно припускав, що batch_ids
    # завжди НОВІ (не перетинаються з уже сканованими) — правильно лише
    # для режиму "початкове покриття". У "циклі освіження" batch_ids —
    # ПІДМНОЖИНА scanned_ids (ті самі SKU пересканують), тож просте
    # додавання подвоїло б рахунок і показувало б >100% чи зростання
    # обсягу, якого немає. Об'єднання множин коректне для ОБОХ режимів.
    scanned_now = len(scanned_ids | set(batch_ids))
    ...
    if mode == "початкове покриття":
        if scanned_now < len(scope):
            print(f"[FullScan] Лишилось {len(scope) - scanned_now} SKU — наступний прогін продовжить звідси.")
        else:
            print("[FullScan] Повне початкове покриття обсягу досягнуто! Наступний прогін перейде "
                  "в цикл освіження (пересканування найстаріших записів).")
    else:
        oldest_refreshed = min((state[pid].get("scanned_at") or "" for pid in batch_ids), default="")
        print(f"[FullScan] Цикл освіження: оновлено {len(batch_ids)} найстаріших записів "
              f"(найстаріший з них був сканований {oldest_refreshed or 'ніколи (стара версія без scanned_at)'}).")
    ...
    mode_label = "початкове покриття" if mode == "початкове покриття" else "цикл освіження"
    send_telegram_message(
        f"🌙 Нічний скан каталогу ({today}, {mode_label}): просканировано {len(batch_ids)}, "
