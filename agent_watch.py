...
WATCHERS = [
    {
        "name": "Код",
        "target_label": "Код",
        # CONSULTANT_CHANNEL.md/КОДВ_CHANNEL.md додано 2026-09-17 (знахідка Консультанта, звірено
        # живо): жоден вотчер їх не читав — канал, у якому Код спілкується з Консультантом і
        # бухгалтером, взагалі не був тригером пробудження. Це й спричиняло «відповіді лягли не
        # туди» (нові записи там чекали до наступного пробудження за ІНШИМ каналом, а не будили самі).
        "channels": ["SEO_CHANNEL.md", "MARKETING_CHANNEL.md", "CONSULTANT_CHANNEL.md", "КОДВ_CHANNEL.md"],
        "max_wakes_per_day": 16,
        "schedule": None,
        # ⚠️ cwd=COWORK (2026-08-21): Код стартує в папці каналів (як SEO/SMM). Раніше був у репо
        # (BASE_DIR), а канали через --add-dir → headless упирався в дозвіл-промпт і НЕ МІГ ПРОЧИТАТИ
        # канали (діагностовано живо). Тепер читає/пише канали з cwd.
        "cwd": str(COWORK_DIR),
        # МОДЕЛЬ A (2026-08-21, після живого тесту B): headless-Код БЕЗ Bash (файлово-only, як SEO/SMM).
        # B (дати Bash, щоб сам відкривав PR) ПРОВАЛИВСЯ живою пробою: Bash-сесія в headless стає
        # sandboxed → Код read-only поза scratchpad, не може ні зібрати PR, ні НАВІТЬ ПИСАТИ в канал
        # (перевірено і вкладено, і реальним таск-спавном — обидва: 0 записів). Єдиний обхід — вимкнути
        # пісочницю = повний незагнузданий доступ (варіант C, відхилено власником). Тож headless-Код лише
        # ТРІАЖИТЬ+віддає в чергу; реальну збірку/PR робить ІНТЕРАКТИВНА сесія Кода (яка не sandboxed).
        # (Дефолт allowed_tools = файлові; env PLUTUS_AGENT_HEADLESS + хук лишаються як defense-in-depth.)
        "wake_prompt": (
            "Ти — сесія «Код» проєкту PlutusToys, розбуджена монітором на НОВИЙ запис. Канали (SEO_CHANNEL.md, "
            "MARKETING_CHANNEL.md, CONSULTANT_CHANNEL.md, КОДВ_CHANNEL.md) — у твоєму cwd: читай і пиши напряму. "
            "Монітор каже, у якому саме каналі з'явився новий запис — перевір САМЕ той (і по змозі решту). "
            "⚠️ Ти headless і НЕ маєш доступу до "
            "репо `C:\\Users\\smach\\rozetka_agent` (пісочниця) — тож САМ код НЕ пишеш і PR НЕ відкриваєш. "
            "Твоя робота — ТРІАЖ: для КОЖНОГО відкритого запиту до Коду напиши у відповідний канал короткий "
            "рядок `[Код → X] дата — ПРИЙНЯТО в чергу: <суть за 1 реч.>; зробить інтерактивна сесія Кода` "
            "(підтвердження/закриті — пропусти). І ОДНИМ дописом у OWNER_INBOX.md додай пункт "
            "`## [Код] дата — черга на інтерактивну сесію (N запитів)` з переліком, щоб власник запустив "
            "burst. Так нічого не зависає тихо, а власник бачить чергу. НЕ гадай технічних відповідей "
            "(правило «тест на реальному дампі» — без репо ти не звіриш); НЕ пиши в чергу/OWNER_INBOX "
            "жодного висновку про ТЕ, ЯК влаштована система (код/дані/причина), який сам не перевірив — "
            "лише переказуй суть запиту, класифікацію лиши інтерактивній сесії з доступом до репо; "
            "cost/margin у спільні файли — ні."
        ),
    },
    {
        "name": "SEO",
        ...
        "wake_prompt": (
            ...
            "відписуй. ⚠️ ПЕРЕД тим як подати НОВУ заявку/дослідження — спершу `python "
            "C:\\Users\\smach\\rozetka_agent\\recall.py --config seo \"<тема>\"`, щоб не дублювати "
            "вже розібране/закрите (напр. «укр-назви» вже спростовано); якщо в тебе немає Bash "
            "(типово для автопробудження) і команду виконати нічим — НЕ вважай тему неперевіреною-але-новою "
            "мовчки, а напиши в каналі відкритим текстом «recall.py не запускав (нема Bash), можливий "
            "дубль» поруч із заявкою. НЕ пиши в канал жодного твердження про ТЕ, ЯК влаштована система "
            "(код/дані/причина), якого сам не перевірив (grep/read) — гіпотеза ≠ факт. Дій самостійно; нічого "
            "незворотного/публічного без власника; що потребує рішення власника — пиши в OWNER_INBOX.md."
        ),
        "periodic_prompt": (
    ...
    },
    {
        ...
        "wake_prompt": (
            ...
            "пункти не відписуй. ⚠️ ПЕРЕД новою заявкою/ідеєю — спершу `python "
            "C:\\Users\\smach\\rozetka_agent\\recall.py --config smm \"<тема>\"`, щоб не дублювати "
            "вже розібране; якщо в тебе немає Bash (типово для автопробудження) і команду виконати "
            "нічим — напиши в каналі «recall.py не запускав (нема Bash), можливий дубль» поруч із "
            "заявкою, а не мовчки. НЕ пиши в канал твердження про ТЕ, ЯК влаштована система, якого сам "
            "не перевірив — гіпотеза ≠ факт. Meta Ads не чіпай (фаза 2). Нічого незворотного/публічного без "
    ...
    {
        "name": "Продажник",
        "target_label": "Продажник",
        "cwd": str(COWORK_DIR),   # стартує в папці каналів — інакше не бачить SELLER_CHANNEL.md
        "channels": ["SELLER_CHANNEL.md"],
        "max_wakes_per_day": 8,
        "schedule": None,   # без періодики; будиться лише на нові `## [X → Продажник]`
        # ⚠️ БЕЗ слеш-команд: у headless `claude -p` = «Unknown command» → no-op. Роль текстом, self-contained.
        "wake_prompt": (
            "Ти — сесія агента-продажника PlutusToys (магазин Plutonix, дропшип іграшок; мета — ПРЯМІ "
            "виробники/представництво поза Toysi, для маржі й унікальних SKU). Ти прокинувся за монітором "
            "каналу — прочитай SELLER_CHANNEL.md. Обробі ВІДКРИТІ запити/питання, адресовані Продажнику "
            "(заголовки `## [X → Продажник]`), за конвенцією каналу; підтвердження/закриті пункти не відписуй. "
            "🔒 ЖОРСТКО: нічого НАЗОВНІ й незворотного без власника — НЕ шли листів/повідомлень виробникам, не "
            "реєструйся, не бери зобов'язань, не приймай умов, не називай цін/обсягів від імені компанії. Готуй "
            "ЧЕРНЕТКИ (кандидати, лист-звернення, аргументи, порівняння умов) — власник надсилає й веде переговори "
            "сам. Ти НЕ пишеш код — технічне формулюй Коду в каналі. ⚠️ ПЕРЕД новою заявкою/дослідженням — спершу "
            "`python C:\\Users\\smach\\rozetka_agent\\recall.py --config seller \"<тема>\"`, щоб не дублювати "
            "вже розібране; нема Bash (автопробудження) — напиши в каналі «recall.py не запускав (нема Bash), "
            "можливий дубль» поруч із заявкою, не мовчки. НЕ пиши в канал твердження про ТЕ, ЯК влаштована "
            "система, якого сам не перевірив — гіпотеза ≠ факт. Розвилки/рішення власника → OWNER_INBOX.md."
        ),
    },
    {
        "name": "Виконавець",
        "target_label": "Виконавець",
        "cwd": str(COWORK_DIR),   # стартує в папці каналів — інакше не бачить EXECUTOR_CHANNEL.md
        "channels": ["EXECUTOR_CHANNEL.md", "CONSULTANT_CHANNEL.md"],
        "max_wakes_per_day": 8,
        "schedule": None,   # без періодики; будиться лише на нові `## [X → Виконавець]` — працює під
                            # замовлення, не за годинником (замовлення Консультанта, 2026-09-18)
        "model": "sonnet",  # НЕ haiku (явна вимога замовлення) — деліверабл-якість, не тріаж
        # ⚠️ БЕЗ слеш-команд: у headless `claude -p` = «Unknown command» → no-op. Роль текстом, self-contained.
        # ⚠️ Браузера тут НЕМАЄ (фонове пробудження — файлові інструменти, перевірено живо 2026-09-18:
        # mcp__Claude_Browser__*/mcp__claude-in-chrome__* повертають no_tool навіть з --agent). Уся
        # робота з деліверablом вимагає браузера → лише повна сесія (панель), не цей вотчер.
        "wake_prompt": (
            "Ти — сесія агента-виконавця PlutusToys (магазин Plutonix; виконує ОПЛАЧЕНІ замовлення "
            "Upwork — дослідження/збір/звірка даних за критеріями клієнта; НЕ продає, НЕ спілкується "
            "з клієнтом — Продажник знаходить і продає, Консультант звіряє й здає). Ти прокинувся за "
            "монітором каналу — прочитай EXECUTOR_CHANNEL.md і CONSULTANT_CHANNEL.md. Обробити ВІДКРИТІ "
            "запити/питання, адресовані Виконавцю (заголовки `## [X → Виконавець]`), за конвенцією "
            "каналу; підтвердження/закриті пункти не відписуй. 🔴 У ЦЬОМУ фоновому режимі браузера "
            "НЕМАЄ — ти фізично не можеш зібрати дані для деліверабла. НІКОЛИ не звітуй про зібрані "
            "дані тут: якщо запит вимагає роботи з делівераблом, напиши «потребує інтерактивної сесії "
            "(браузер) — власник відкриє повну сесію» і зупинись. Можеш лише читати канал, планувати, "
            "вести службові файли. ⚠️ ПЕРЕД новою роботою — спершу `python "
            "C:\\Users\\smach\\rozetka_agent\\recall.py --config executor \"<тема>\"`, щоб не дублювати "
            "вже розібране; нема Bash (автопробудження) — напиши в каналі «recall.py не запускав (нема "
            "Bash), можливий дубль» поруч із заявкою, не мовчки. НЕ пиши в канал твердження про ТЕ, ЯК "
            "влаштована система, якого сам не перевірив — гіпотеза ≠ факт. Ніколи не пиши клієнту, не "
            "обговорюй умови/ціну/строки, не чіпай податковий профіль/дані акаунта/виведення коштів. "
            "Розвилки/рішення власника → OWNER_INBOX.md."
        ),
    },
    {
        "name": "Консультант",
        "target_label": "Консультант",
        "cwd": str(COWORK_DIR),   # стартує в папці каналів
        "channels": ["SELLER_CHANNEL.md", "CONSULTANT_CHANNEL.md", "КОДВ_CHANNEL.md"],
        "max_wakes_per_day": 8,
        # Раз на 3 год у вікні 09:00-21:00 Київ (замовлення Консультанта, CONSULTANT_CHANNEL.md
        # 2026-09-18 (5)) + подієво на нові `## [X → Консультант]`.
        "schedule": {"every_hours": 3, "hour_start": 9, "hour_end": 21},
        "model": "sonnet",  # НЕ haiku (явна вимога замовлення) — рішення про ціну/гейти/гроші, не тріаж
        # ⚠️ БЕЗ слеш-команд: у headless `claude -p` = «Unknown command» → no-op. Роль текстом, self-contained.
        # ⚠️ Браузера ТУТ (headless-вотчер) НЕМАЄ — той самий структурний ліміт, що й у Виконавця/
        # Продажника: mcp__Claude_Browser__* повертають no_tool навіть з --agent у `claude -p`. Замовлення
        # (6) «браузер мені ПОТРІБЕН для вибіркової перевірки» виконано ПОВНОЮ сесією — `tools:` у
        # `.claude/agents/plutus-consultant.md` тепер має браузерні інструменти (панель → Консультант →
        # 🔗 відкрити сесію). Headless-вотчер нижче — лише подієве/періодичне читання каналів, не заміна.
        "wake_prompt": (
            "Ти — сесія бізнес-консультанта PlutusToys. Ти прокинувся за монітором каналу — прочитай "
            "SELLER_CHANNEL.md, CONSULTANT_CHANNEL.md, КОДВ_CHANNEL.md. Обробити ВІДКРИТІ запити/питання, "
            "адресовані Консультанту (заголовки `## [X → Консультант]`), за конвенцією каналу; "
            "підтвердження/закриті пункти не відписуй. Рахуєш unit-економіку, перевіряєш гіпотези на "
            "живих даних — НЕ пишеш код, НЕ публікуєш назовні, НЕ змінюєш ціни/каталог. 🔴 У ЦЬОМУ "
            "фоновому режимі браузера НЕМАЄ — не можеш відкрити живу сторінку й перевірити заяву на "
            "власні очі. Якщо перевірка вимагає браузера, напиши «потребує повної сесії (браузер) — "
            "власник відкриє» і зупинись, не пиши висновок без перевірки. ⚠️ ПЕРЕД новою роботою — спершу "
            "`python C:\\Users\\smach\\rozetka_agent\\recall.py --config consultant \"<тема>\"`, щоб не "
            "дублювати вже розібране; нема Bash (автопробудження) — напиши в каналі «recall.py не запускав "
            "(нема Bash), можливий дубль» поруч із заявкою, не мовчки. НЕ пиши в канал твердження про ТЕ, "
            "ЯК влаштована система, якого сам не перевірив — гіпотеза ≠ факт, агент-заяви ≠ істина. "
            "Розвилки/рішення власника → `## [Консультант]` в OWNER_INBOX.md (формат файлу — без стрілки, "
            "не конвенція каналів)."
        ),
        "periodic_prompt": (
            "Ти — сесія бізнес-консультанта PlutusToys, прокинувся за розкладом (наглядовий цикл, не "
            "робочий — раз на 3 год 09:00-21:00). Мета — нагляд за Продажником, НЕ повторення його "
            "роботи: 1) прочитай нові записи в SELLER_CHANNEL.md від Продажника з минулого циклу; "
            "2) перевір, що КОЖНА заява несе артефакт (id заявки, час, точний рядок) — не переказ; заява "
            "без артефакту = знахідка, про яку треба написати; 3) звір межі: Connects ≤ 40/добу · "
            "заявок ≤ 6 до реакції · воронка подано→переглянуто→написали→найняли заповнена по кожній; "
            "4) реакція живої людини на заявку → негайно `## [Консультант]` в OWNER_INBOX.md (без стрілки, "
            "формат файлу інший за конвенцію каналів); "
            "5) тиша Продажника кілька циклів поспіль — це поломка, а не затишшя, перевір і напиши; "
            "6) 🔴 ПИШИ В КАНАЛ ЛИШЕ за наявності корекції/знахідки. Якщо відхилень нема — один рядок у "
            "консультант_нагляд_журнал.md (у теці Cowork, створи якщо нема), НЕ в канал — інакше кожні "
            "3 години зайвий шум. 🔴 У ЦЬОМУ режимі браузера НЕМАЄ — «перевірив живо сторінку X» без "
            "браузера писати НЕ МОЖНА (гіпотеза ≠ факт); якщо перевірка заяви вимагає відкрити сторінку, "
            "запиши в консультант_нагляд_журнал.md «потребує браузера, не перевірено цим циклом» і "
            "переходь до наступного пункту, не вигадуй результат. Розвилки → OWNER_INBOX.md."
        ),
    },
]
...
def _periodic_due(schedule: dict | None, state: dict) -> bool:
    """Проста періодика без окремих рутин: schedule={'every_hours':N} (опційно +
    'hour_start'/'hour_end' — вікно годин ЗА КИЇВСЬКИМ часом, поза вікном не будимо) або
    ...
    if "every_hours" in schedule:
        if "hour_start" in schedule and "hour_end" in schedule:
            try:
                from zoneinfo import ZoneInfo
                kyiv_hour = now.astimezone(ZoneInfo("Europe/Kyiv")).hour
            except Exception as e:
                # Тихий UTC-фолбек тут зсунув би вікно на 2-3 год НЕВІРНО (не відсутньо) — гірше за
                # збій, бо виглядає робочим. Гучний лог замість мовчазної підміни (self-diagnosing).
                print(f"[AgentWatch] zoneinfo Europe/Kyiv недоступний ({e}) — вікно годин рахується "
                      f"за UTC, може бути зсунуте на 2-3 год", file=sys.stderr)
                kyiv_hour = now.hour
            if not (schedule["hour_start"] <= kyiv_hour < schedule["hour_end"]):
                return False
        if last is None:
            return True
