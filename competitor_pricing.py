...
# формула не знає про фіксовані, В ГРИВНЯХ витрати на замовлення, відомі
# з 2026-07-13 (pt74/pt81, PlutusToys_avtonomiya_plan.md):
#   - пакування 15₴ при сумі замовлення <1000₴ (безкоштовне вище) —
#     ⚠️ ДОСІ НЕ ПІДТВЕРДЖЕНО, чи це вже включено в собівартість Toysi,
#     чи виставляється окремою статтею в накладній (звірка з реальною
#     накладною — на власниці).
...
#
# compute_floor() нижче свідомо трактує ОБИДВІ як ЗАВЖДИ можливі (сума —
# верхня, "гіршого випадку" оцінка, не гарантована для кожного продажу):
# краще трохи завищити floor на замовленні з каталогу без пакувальної
# доплати, ніж пропустити реальний збиток там, де обидві витрати
# насправді стягуються. Скоригувати вниз, коли пакування підтвердиться
# включеним у собівартість Toysi.
PACKAGING_COST_UAH = 15.0
