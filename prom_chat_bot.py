...
# ЄДИНЕ СПІЛЬНЕ ДЖЕРЕЛО скарг/дефекту/невдоволення — укр.+рос. (Prom часто рос.-мовний). Вживається
# ОБОМА гейтами: гейтом подяки (_ACK_STOP_MARKERS = питання + ЦЕЙ набір) і гейтом шаблону
# (_has_complaint). Раніше два гейти мали РІЗНІ списки й дрейфували — аудит #484 тричі ловив теч, де
# _ACK_STOP уже мав стем (якіс/поган/«не то »), а гейт шаблону — ні. Тепер обидва з одного джерела.
# Якщо є хоч один — НЕ шаблонимо й НЕ дякуємо, віддаємо на ескалацію. Ширший набір, ніж помилитися в
# інший бік. Голе «бит»/«ціна» НЕ беремо (ловило б «орбіта»/питання ціни) — лише конкретні форми.
_COMPLAINT_MARKERS = (
    # контраст: «позитив-стем … АЛЕ … (ускладнення)» — ловить оптом, навіть з дефект-словом не зі списку.
    " но ", " но,", " але ", " але,", "однак", "хоча", "хотя",
    # повернення / гарантія
    "поверн", "верну", "возврат", "вернит", "гаранті", "гаранти",
    # не працює
    "не працю", "не робот", "не работ", "несправн", "неисправ",
    # зламане / пошкоджене / дефект (укр+рос форми)
    "брак", "бракован", "злама", "сломал", "сломан", "розбит", "разбит", "битое", "битый", "битая",
    "битые", "пошкодж", "поврежд", "повредж", "испорч", "зіпсу", "дефект", "мят", "м'ят", "брудн",
    "грязн", "поцарап", "царапин", "подряпан", "подряпин", "зіпс", "испорт", "порв", "надір", "надорв", "рван",
    # якість
    "якіс", "якост", "качеств", "некачеств", "плох", "погано", "погана", "поган", "отвратительн", "хлам",
    # затримка / не дійшло / загубилось
    "запізн", "опозда", "затрим", "задерж", "не пришл", "не пришёл", "не дошл", "не дошёл", "не прийш",
    "не дійш", "не відправ", "не отправ", "не надісл", "не отдал", "загуб", "втрат", "потерял",
    "де моє", "де замовл", "де моя посилк",
    # не той / не підходить / невідповідність / недостача / підробка
    "не той", "не таке", "не те", "не то ", "не тот", "не така", "не так", "не підход", "не подход",
    "не соответств", "не відповід", "недостач", "не вистача", "не хвата", "не вклали", "не доклали",
    "не поклали", "не полож", "подделк", "завышен",
    # невдоволення / претензія / обман / обмін
    "скарг", "жалоб", "претенз", "проблем", "не сподоб", "не понрав", "розчаров", "разочаров",
    "обурен", "жахлив", "ужасн", "недовол", "обман", "мошенн", "кинули меня", "обмін", "чому", "почему",
)
...
def _has_complaint(normalized: str) -> bool:
    ...
    return any(m in normalized for m in _COMPLAINT_MARKERS)
...
# Питання-маркери — блокують ЛИШЕ авто-подяку (не хочемо «дякувати» на питання), АЛЕ дозволяють
# шаблонну відповідь (гейт шаблону їх не вживає — саме на «наявн/ціна/коли» шаблон і відповідає).
_QUESTION_MARKERS = (
    "?", "чи ", "як ", "коли", "скільки", " де ", "можна", "хочу", "потрібно", "підкажіт",
    "цікавить", "наявн", "колір", "кольор", "розмір", "ціна", "вартіст", "оплат", "цікаво",
    "хотіла б", "хотів би", "тільки ", "только ",
)
# Гейт подяки = питання АБО скарга (спільний _COMPLAINT_MARKERS). Так подяка й шаблон беруть скарги з
# ОДНОГО джерела — списки більше не дрейфують.
_ACK_STOP_MARKERS = _QUESTION_MARKERS + _COMPLAINT_MARKERS
ACK_ANSWER = (
