...
# ТИМЧАСОВО ПІДНЯТО до 750 (2026-07-19, пряме рішення власниці): рейс-
# стан (PR #102, вже виправлено) обнулив _delisted_since для рівно тих
# самих 722 товарів, що й спричинили початковий інцидент 970→244 —
# вони зараз знову живі на Prom, і звичайний ліміт 150 назавжди
# блокував би --apply без ручного втручання (722 > 150, sys.exit(1)
# щоразу, до безкінечності). Власниця свідомо дозволила один
# контрольований прогін із запасом (750 > 722, "щоб напевне") для
# повторного видалення САМЕ ЦИХ товарів, тепер безпечно (рейс-стан
# закрито, паралельних прогонів більше не буде). ПОВЕРНУТИ до 150
# ОКРЕМИМ PR одразу після цього прогону — 750 як постійне значення
# більше НЕ захищає від повторення оригінального інциденту (722 сталось
# один раз, 750 його б пропустило).
MAX_DELIST_PER_RUN = 750
...
def delist(external_id: str) -> None:
    """ОСТАТОЧНЕ рішення власниці (2026-07-18): не приховувати
    (presence="not_available" — коротко розглядався, живо перевірявся
    на SKU 265230 і відкочений), а видаляти повністю. Пряме
    обґрунтування власниці: "якщо вони не конкурентні вже, то
    конкурентними не стануть" — тримати непродаваний, неконкурентний
    лістинг "про запас" немає сенсу.

    status="deleted" — той самий виклик, що й prom_catalog_sync.py
