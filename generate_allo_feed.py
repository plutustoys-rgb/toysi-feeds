...
ЦІНА АЛЛО = ЦІНА PROM (2026-07-23, пряме рішення власниці): раніше тут
рахувалась незалежна ціна через decide_price_for_platform() з окремим
запобіжником "не більше ніж +20% над Prom" (ризик цінового паритету
документації АЛЛО — "ціна на ресурсі партнера = ціні на allo.ua",
20%+ завищення -> пониження рейтингу/"Призупинено"). Власниця вказала
на просте й точне обґрунтування: максимальна комісія АЛЛО за категорією
"іграшки" — 20%, максимальна комісія Prom — 25% (PROM_CATEGORY_ID_
COMMISSION, id=29 "Товари, загальне"). Ціна Prom вже гарантує 3%
мінімальну маржу ПОНАД комісію до 25% — та сама ціна на АЛЛО (реальна
комісія якого ніколи не вища за 20%) структурно ніколи не може дати
гірший результат, лише кращий. Тому: жодної окремої формули ціни для
АЛЛО, жодного цінового паритету (він природно виконується — ціна
ідентична Prom, не може бути "вищою за Prom на 20%"). Той самий
принцип і той самий код-патерн, що вже застосований для EVA (PR #141)
і Google/Meta/Bing (PR #143) — АЛЛО просто приєднується до вже наявної
конвенції "усі майданчики, крім Rozetka, = ціна Prom".
...
(курований топ), що й Prom/EVA. Але фінальний список ЗАМОРОЖЕНИЙ статичним знімком
...
def _qualifies_for_feed(item: dict, excluded: set, prom_price_overrides: dict) -> bool:
    ...
    в generate_eva_feed.py::_qualifies_for_feed(). ЦІНА АЛЛО = ЦІНА PROM
    (2026-07-23, пряме рішення власниці, див. докстрінг файлу) — АЛЛО
    більше не рахує ціну незалежно через decide_price_for_platform():
    товар без свіжого prom_price_overrides запису просто не потрапляє у
    фід. allo_floor нижче — дешевий структурний запобіжник (той самий
    архітектурний патерн, що eva_floor у generate_eva_feed.py), а не
    очікувана точка спрацювання: комісія АЛЛО (макс. 20%) ніколи не вища
    за комісію Prom (макс. 25%), тож ціна Prom апріорі покриває мінімальну
    маржу АЛЛО з запасом."""
    try:
    ...
    if prom_price is None:
        return False
    allo_floor = compute_floor(cost, get_platform_commission(PLATFORM), MIN_PROFIT)
    if prom_price < allo_floor:
        return False
...
def _build_xml(
    ...
    for item in catalog.values():
        ...
        # ЦІНА АЛЛО = ЦІНА PROM (докстрінг файлу) — АЛЛО більше НЕ рахує
        # ціну незалежно (decide_price_for_platform), копіює свіжу ціну
        # Prom з того ж прогону update-feeds.yml (load_fresh_prom_price_
        # overrides). Товар без свіжого запису просто не потрапляє у фід.
        ...
        pictures = clean_photos.pictures_for(item, ALLO_MAX_PICTURES)
        if not pictures:
            skipped_no_pics += 1
            continue
    ...
    print(f"[ALLO] У фіді: {len(offers_el)} товарів | "
          ...
          f"виключено вручну/нерентабельно під комісією АЛЛО: {skipped_unprof} | "
