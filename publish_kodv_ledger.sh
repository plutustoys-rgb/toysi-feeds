...
# ⛔ ВИМКНЕНО 2026-08-10 (фінансовий витік): `kodv-ledger-data` — гілка ПУБЛІЧНОГО
# репо (feed-data мусить лишатись публічним, бо Google/Meta тягнуть фіди без токена),
# тож branch-«ізоляція» приватності НЕ давала — цей ФІНАНСОВИЙ журнал (дата, №замовлення,
# виручка, собівартість, комісія, назви товарів) читався будь-ким за raw-URL. Реальний
# витік того ж класу, що NovaPay. Публічний пуш ЗУПИНЕНО; гілку kodv-ledger-data видалено.
# Локальний /opt/plutustoys/kodv_ledger.jsonl (джерело правди) лишається недоторканим;
# сама книга KODV_*.xlsx — локальна в власника, не зачеплена. Дохід для «агента-
# бухгалтера» доставлятиметься ПРИВАТНИМ каналом (окремий скрипт). Тіло нижче лишено
# для історії й НЕ виконується.
echo "[publish_kodv_ledger] ВИМКНЕНО: публічна публікація КОДВ-журналу зупинена (витік фінданих)." >&2
exit 0
