...
def _append_kodv_ledger_entries(conn, catalog: dict) -> int:
    ...
    # ВИПРАВЛЕНО (2026-07-20, аудит PR #106 — code_report_2026-07-20_pt2.md):
    # явний conn.commit() ТУТ, до запису у файл — раніше позначка
    # kodv_logged_at лишалась лише в черзі транзакції аж до завершення
    # ЗОВНІШНЬОГО `with get_connection()` у build_report() (набагато пізніше,
    # після решти роботи звіту) — крах процесу МІЖ фізичним записом у
    # kodv_ledger.jsonl і тим комітом дав би ПОСТІЙНИЙ дублікат при
    # наступному прогоні (bо kodv_logged_at так і не збережеться). Тепер
    # вікно ризику — лише між цим commit() і f.write() нижче (набагато
    # вужче); найгірший наслідок такого рідкісного краху — один пропущений
    # запис (виявний звіркою кількості рядків файлу з кількістю замовлень
    # kodv_logged_at у БД), не мовчазний дублікат у git-історії журналу.
    conn.commit()
