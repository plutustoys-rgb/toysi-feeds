...
_DELIVERY_STEMS = (
    ...
    # рос (Prom рос.-важкий; «доставк» уже спільна) — питання, не скарги
    "отправ", "почтой", "новой почт", "какой почт", "перевозчик", "с какого города",
)
_STOCK_STEMS = ("наявн", "залишок", "є в наявн", "в наявн", "наличи", "остаток", "остатке", "в остатке")
_PRICE_STEMS = (
    "яка ціна", "скільки кошту", "вартіст", "почім", "по чому", "ціна товар", "ціна на",
    "сколько стоит", "стоимость", "какая цена", "цена товара",
)
...
_TEMPLATE_COMPLAINT_MARKERS = (
    ...
    # рос. форми, які НЕ покриваються укр. стемами вище (Prom дуже часто рос.-мовний; «доставка»
    # однакова укр/рос, тож рос. скарга про доставку інакше протекла б у бадьору відповідь):
    "возврат", "вернит", "не пришл", "не пришёл", "не дошл", "не дошёл", "обман", "мошенн", "кинули меня",
    "не отправ", "не отдал", "потерял",
    # рос. дефект/якість/невідповідність — форми, які НЕ покриті укр. стемами вище (2-е коло аудиту
    # #484: рос.-розширення стемів наявності/ціни/доставки відкрило теч, бо стем ловив іменник, а
    # гейт пропускав дефект). Голе «бит» НЕ беремо (ловило б «орбіта») — лише конкретні форми.
    "сломан", "не работ", "испорч", "битое", "битый", "битая", "битые", "некачеств", "качеств",
    "завышен", "не соответств", "не відповід", "подделк", "отвратительн", "поцарап", "подряпан",
    # контраст-слова: «в наявності/ціна/доставка … АЛЕ …» — майже завжди ускладнення/скарга. Ловить
    # оптом усі «позитив-стем але дефект», навіть з дефект-словом, якого нема в списку вище. Компаунд-
    # питання («є, але коли відправка?») теж піде на ескалацію — це безпечно (людина відповість повно).
    " но ", " але ", "однак", "хоча", "хотя",
)


# Прохання надіслати медіа — бот їх виконати не може (фото/відео товару), тому НЕ шаблонимо
# (інакше «відправте/отправьте фото» ловився б стемом доставки «відправ»/«отправ» і давав відповідь
# про доставку). Йде на ескалацію до власника.
_MEDIA_REQUEST_MARKERS = ("фото", "видео", "відео", "видос", "відос", "картинк", "знімок")
...
def _has_media_request(normalized: str) -> bool:
    """True, якщо просять фото/відео — бот їх не надасть, шаблон НЕ застосовуємо (ескалація)."""
    return any(m in normalized for m in _MEDIA_REQUEST_MARKERS)
...
def try_template_response(customer_message: str, product_context: dict | None) -> str | None:
    ...
    # Скарга/дефект/повернення/затримка АБО прохання медіа → НЕ шаблонимо жодним шаблоном,
    # віддаємо на ескалацію (бадьора відповідь на скаргу = шкода; фото/відео бот не надасть).
    # Стоїть ПЕРЕД усіма шаблонами нижче.
    if _has_complaint(normalized) or _has_media_request(normalized):
        return None
