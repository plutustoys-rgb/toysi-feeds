...
PHASH_MAX_DISTANCE = 12        # ≤ цього = «той самий товар» (калібрувати)
...
def rozetka_market_price(page, session, name: str, our_hash) -> float | None:
    """Публічний пошук Rozetka за назвою (Playwright рендерить плитки) → pHash-звірка фото →
    min ціна серед підтверджених «той самий товар». None = нема підтвердженого конкурента."""
    ...
    try:
        ...
        try:
            ...
            return None   # нема плиток (нема товару на Rozetka) → унікальний
...
def run(catalog: dict, membership: set, prom_products: dict, limit: int) -> tuple[list, dict]:
    ...
    st = {"eligible": 0, "checked": 0, "competitive": 0, "uncompetitive": 0, "unique": 0,
          "oos_or_lowstock": 0, "already": 0, "invalid_card": 0, "no_clean_image": 0, "bad_cost": 0}
    ...
    with sync_playwright() as p:
        ...
        ctx = browser.new_context(user_agent=HEADERS["User-Agent"], locale="uk-UA")
        page = ctx.new_page()
