...
def fetch_buybox_competitor(prom_id: int, url_text: str) -> dict | None:
    ...
    try:
        ...
        if not match:
            # ⚠️ ПЕРЕГЛЯНУТО 2026-09-23 (живе розслідування флагованого "buyBox parsing
            # bug"): попередня версія друкувала тут ⚠️ на кожен промах, вважаючи "поле
            # взагалі не знайдено" рідкісним сигналом можливої зміни розмітки Prom. Живо
            # перевірено на 60 випадкових SKU нашого каталогу (requests.get, той самий
            # шлях, що тут): поле ВІДСУТНЄ на ~50% живих (нередіректнутих) сторінок
            # товару — це НОВА НОРМА, не аномалія. Крос-звірка з історичним
            # prom_competitor_price_state.json (09.09, 5 тижнів тому) показала: 0 із 30
            # таких SKU КОЛИСЬ мали підтвердженого buybox-конкурента — тобто Prom,
            # найімовірніше, просто НЕ рендерить блок для товарів без жодного іншого
            # продавця тієї самої моделі (раніше, вочевидь, рендерив і з companyCount=0).
            # Функціонально різниці нема: обидва шляхи повертають None, той самий фолбек.
            # Друк на кожен промах перетворився на постійний шум без сигналу — прибрано.
            # Реальний грубий сигнал (масове зламання regex) лишається в агрегованому
            # buybox_count/buybox_attempted_count підсумку прогону (нижче в файлі) —
            # дивись його, якщо колись справді знадобиться діагностувати регресію.
            return None
...
#
# ЗБІЛЬШЕНО 500 -> 2500 (2026-07-29, пряме прохання власниці — "маржу
# 3% не змінюємо, пришвидшити recheck backlog"): 500 НІКОЛИ не було
# точним розрахунком проти реального ліміту — консервативне кругле
# число, обране лише щоб зняти оригінальний баг (необмежений прохід).
# Реальне обмеження — ЧАСОВИЙ БЮДЖЕТ прогону (MAX_RUNTIME_SECONDS,
# спільний з ROTATION_BATCH_SIZE через _time_budget_exceeded() —
# recheck йде ПЕРШИМ у main(), тож великий ліміт тут забирає час у
# основного ротаційного циклу, не навпаки), НЕ зовнішній rate-limit
# Prom (SEARCH_DELAY=0.4с — власна ввічливість, не вимога API). За тією
# ж оцінкою ~1-2с/SKU, що й LIVE_LOOKUP_EXTRA_BATCH_LIMIT вище: 2500 —
# це ~42-83 хв додатково на прогін, залишає GH-Actions-ери 5-годинний
# MAX_RUNTIME_SECONDS з великим запасом (і тим більше — VPS-таймер
# Фази 2, де вже немає 6-годинної стелі GH Actions взагалі). Бектлог
# 9929 (2026-07-29) — 20 прогонів по 500 -> 4 по 2500. `_time_budget_
# exceeded()` лишається останнім запобіжником незалежно від цього
# числа: якщо реальна мережа повільніша за оцінку, цикл сам обірветься
# на дедлайні, а не перевищить бюджет. Якщо recheck переїде на окремий,
# частіший VPS-таймер (Фаза 2) — цей ліміт і саме це обмеження стають
# незастосовними, розглянути окремим PR.
RECHECK_DELISTED_BATCH_LIMIT = 2500
