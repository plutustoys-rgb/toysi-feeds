...
EVA_EXCLUDED_CATEGORIES = {
    ...
    # Уцінка
    "51995",
    # ДОДАНО (2026-09-01, «дочисти EVA»): активні не-іграшки, що ще текли у фід і давали
    # «Категорія EVA: —» (verify_eva_category_map фолбеки). Той самий клас, що вже виключений:
    # Новорічний декор + Ялинки/сосни (сезонний декор, група C), Косметичні засоби (косметика),
    # Прання та миття посуду (побутхімія). Не-іграшки в EVA-фід не йдуть.
    "98917", "98920", "99242", "99244",
}
...
# (без decide_price_for_platform; НЕ універсальний compute_floor, а точкова покривна підлога 5%
# ЛИШЕ для збиткових — PR #438). У фід беруться ВСІ валідні товари в наявності,
...
def _eva_price(item: dict):
    ...
    ПОКРИВНА ПІДЛОГА (2026-08-30, PR #438): без Збірки в ціні дешеві товари (база < ~55 грн)
    йшли в збиток (нетто після 15% комісії EVA < собівартості); тепер ТАКІ (і лише такі)
    піднімаються до 5% маржі над собівартістю (cost×1.05/0.85). Прибуткові й промо — без змін."""
    try:
        cost = real_toysi_cost(item)          # реальна собівартість (зі Збіркою) — для валідності
        base = toysi_discounted_price(item)   # ціна тойсі з нашою знижкою (БЕЗ Збірки) — база ×1.5
    except (ValueError, TypeError):
        return None
...
def _build_xml(
    ...
    for item in catalog.values():
        ...
        # ЦІНА EVA — це вже пораховані _eva_price() ціни (ціна тойсі зі знижкою БЕЗ Збірки ×
        # EVA_PRICE_MULTIPLIER=1.5, або промо-множник), передані з _build_eva_live_selection().
        # Кожен відібраний SKU має ціну; item без запису сюди не доходить (відсіяний ще у відборі)
        # — цей guard лишається як страхувальний від None.
        # НЕ універсальний compute_floor (він відкинув би пів-фіду — cost×1.5 близько cost×1.47-межі).
        # Натомість _eva_price має ТОЧКОВУ покривну підлогу 5% ЛИШЕ для збиткових (пласка Збірка
        # підминає ×1.5 на дешевих товарах) — рішення власника 2026-08-30, PR #438.
